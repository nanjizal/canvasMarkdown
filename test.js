...
var TestMarkdownCanvas = function() {
	this.h = 4000;
	this.w = 800;
	this.canvasMarkdown = new canvasMarkdown_CanvasMarkdown(10,10,this.w,this.h);
	this.canvasMarkdown.add("An h1 header\n============\n\nParagraphs are separated by a blank line.\n\n2nd paragraph. *Italic*, **bold**, and `monospace`. Itemized lists\nlook like:\n\n  * this one\n  * that one\n  * the other one\n\nNote that --- not considering the asterisk --- the actual text\ncontent starts at 4-columns in.\n\n> Block quotes are\n> written like so.\n>\n> They can span multiple paragraphs,\n> if you like.\n\nUse 3 dashes for an em-dash. Use 2 dashes for ranges (ex., \"it`s all\nin chapters 12--14\"). Three dots ... will be converted to an ellipsis.\nUnicode is supported. ☺\n\n\n\nAn h2 header\n------------\n\nHere`s a numbered list:\n\n 1. first item\n 2. second item\n 3. third item\n\nNote again how the actual text starts at 4 columns in (4 characters\nfrom the left side). Here`s a code sample:\n\n    # Let me re-iterate ...\n    for i in 1 .. 10 { do-something(i) }\n\nAs you probably guessed, indented 4 spaces. By the way, instead of\nindenting the block, you can use delimited blocks, if you like:\n\n~~~\ndefine foobar() {\n    print \"Welcome to flavor country!\";\n}\n~~~\n\n(which makes copying & pasting easier). You can optionally mark the\ndelimited block for Pandoc to syntax highlight it:\n\n~~~python\nimport time\n# Quick, count to ten!\nfor i in range(10):\n    # (but not *too* quick)\n    time.sleep(0.5)\n    print i\n~~~\n\n\n\n### An h3 header ###\n\nNow a nested list:\n\n 1. First, get these ingredients:\n\n      * carrots\n      * celery\n      * lentils\n\n 2. Boil some water.\n\n 3. Dump everything in the pot and follow\n    this algorithm:\n\n        find wooden spoon\n        uncover pot\n        stir\n        cover pot\n        balance wooden spoon precariously on pot handle\n        wait 10 minutes\n        goto first step (or shut off burner when done)\n\n    Do not bump wooden spoon or it will fall.\n\nNotice again how text always lines up on 4-space indents (including\nthat last line which continues item 3 above).\n\nHere`s a link to [a website](http://foo.bar), to a [local\ndoc](local-doc.html), and to a [section heading in the current\ndoc](#an-h2-header). Here`s a footnote [^1].\n\n[^1]: Footnote text goes here.\n\nTables can look like this:\n\nsize  material      color\n----  ------------  ------------\n9     leather       brown\n10    hemp canvas   natural\n11    glass         transparent\n\nTable: Shoes, their sizes, and what they`re made of\n\n(The above is the caption for the table.) Pandoc also supports\nmulti-line tables:\n\n--------  -----------------------\nkeyword   text\n--------  -----------------------\nred       Sunsets, apples, and\n          other red or reddish\n          things.\n\ngreen     Leaves, grass, frogs\n          and other things it`s\n          not easy being.\n--------  -----------------------\n\nA horizontal rule follows.\n\n***\n\nHere`s a definition list:\n\napples\n  : Good for making applesauce.\noranges\n  : Citrus!\ntomatoes\n  : There`s no \"e\" in tomatoe.\n\nAgain, text is indented 4 spaces. (Put a blank line between each\nterm/definition pair to spread things out more.)\n\nHere`s a \"line block\":\n\n| Line one\n|   Line too\n| Line tree\n\nand images can be specified like so:\n\n![example image](example-image.jpg \"An exemplary image\")\n\nAnd note that you can backslash-escape any punctuation characters",$bind(this,this.loaded));
};
TestMarkdownCanvas.__name__ = true;
TestMarkdownCanvas.main = function() {
	new TestMarkdownCanvas();
};
TestMarkdownCanvas.prototype = {
	loaded: function() {
		this.canvasMarkdown.transformFunction($bind(this,this.transformation));
	}
	,transformation: function(dataIn,w_,h_) {
		var count = 0;
		var _g1 = 0;
		var _g = w_ * h_;
		while(_g1 < _g) {
	...
	,__class__: TestMarkdownCanvas
};
var canvasMarkdown_CanvasMarkdown = function(x_,y_,w_,h_) {
	...
	this.svgRoot = this1;
	var width_ = this.w;
	this.svgRoot.setAttribute("width",width_ == null?"null":"" + width_);
	var height_ = this.h;
	this.svgRoot.setAttribute("height",height_ == null?"null":"" + height_);
	this.body.removeChild(this.svgRoot);
	var this2;
	var canvas = window.document.createElement("canvas");
	var style1 = canvas.style;
	style1.paddingLeft = "0px";
	style1.paddingTop = "0px";
	style1.left = "0px";
	style1.top = "0px";
	...
	this.canvas.style.left = (x_ == null?"null":"" + x_) + "px";
	this.canvas.style.left = (y_ == null?"null":"" + y_) + "px";
	this.ctx = this.canvas.getContext("2d",null);
};
canvasMarkdown_CanvasMarkdown.__name__ = true;
canvasMarkdown_CanvasMarkdown.prototype = {
	add: function(md_,loaded_) {
		this.md = md_;
		this.loaded = loaded_;
		this.createForeign(this.md);
		var svg = this.svgRoot;
		this.url = URL.createObjectURL(new Blob([new XMLSerializer().serializeToString(svg)],{ type : "image/svg+xml;charset=utf-8"}));
		this.img.onload = $bind(this,this.svgAsImageLoaded);
		this.img.src = this.url;
	}
	,transformFunction: function(f) {
		var imageData = this.ctx.getImageData(0,0,this.w,this.h);
		f(new Uint8ClampedArray(imageData.data.buffer),this.w,this.h);
		this.ctx.putImageData(imageData,0,0);
		window.document.body.appendChild(js_Boot.__cast(this.canvas , Node));
	}
	,svgAsImageLoaded: function() {
	...
	,createForeign: function(content_) {
		var svgForeign = window.document.createElementNS("http://www.w3.org/2000/svg","foreignObject");
		svgForeign.setAttribute("x","0");
		svgForeign.setAttribute("y","0");
		canvasMarkdown__$SvgForeign_SvgForeign_$Impl_$.markDown(svgForeign,content_);
		this.svgRoot.appendChild(svgForeign);
		return svgForeign;
	}
	,__class__: canvasMarkdown_CanvasMarkdown
};
var canvasMarkdown__$SvgForeign_SvgForeign_$Impl_$ = {};
canvasMarkdown__$SvgForeign_SvgForeign_$Impl_$.__name__ = true;
canvasMarkdown__$SvgForeign_SvgForeign_$Impl_$.markDown = function(this1,md) {
	var doc = window.document;
	var div = doc.createElement("div");
	div.innerHTML = "<FONT FACE=\"Trebuchet MS, Helvetica, sans-serif\">" + Markdown.markdownToHtml(md) + "</FONT>";
	doc.body.appendChild(div);
	var width_ = div.offsetWidth | 0;
	this1.setAttribute("width",width_ == null?"null":"" + width_);
	var height_ = div.offsetHeight | 0;
	this1.setAttribute("height",height_ == null?"null":"" + height_);
	doc.body.removeChild(div);
	this1.appendChild(js_Boot.__cast(div , Node));
};
